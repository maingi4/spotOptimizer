...
import * as c from '../contracts/optimizeRs';
import * as e from '../contracts/error';
import * as cw from './cloudWatchConnector';

export namespace AWS {
    export class autoScaleConnector {

        constructor(private autoScaleGroupName: string,
            private regionName: string, private awsAccessKeyId: string,
            private awsSecretAccessKey: string) { }

        getAutoScaleInstanceHistory(daysToConsider: number,
            callback: (history: c.Contracts.Schedule, error: e.Contracts.Error) => void): void {
            ...
            var me = this;
            cwConnector.getMetrics(daysToConsider, "AWS/AutoScaling", metricName, 60 * 60,
                cw.AWS.StatisticType.Maximum, [new cw.AWS.CWFilter("AutoScalingGroupName", groupName)],
                function (metrics: Array<cw.AWS.Metric>, ex: e.Contracts.Error) {
                    if (ex) {
                        callback(null, ex);
                        return;
                    }
                    ...
                    var avgByHour = _.chain(metrics)
                        .groupBy('hour')
                        .map(function (value: Array<cw.AWS.Metric>, key: string) {
                            var sum = 0;
        ...
        getCPUMetricSchedule(daysToLookBack: number, secondsAtWhichScalingHappens: number,
            callback: (cpuHistory: c.Contracts.Schedule, error: e.Contracts.Error) => void): void {
            var cwConnector = new cw.AWS.cloudWatchConnector(this.regionName, this.awsAccessKeyId, this.awsSecretAccessKey);

            var groupName = this.autoScaleGroupName;
            var metricName = "CPUUtilization";

            cwConnector.getMetrics(daysToLookBack, "AWS/EC2", metricName, secondsAtWhichScalingHappens,
                cw.AWS.StatisticType.Average, [new cw.AWS.CWFilter("AutoScalingGroupName", groupName)],
                function (metrics: Array<cw.AWS.Metric>, ex: e.Contracts.Error) {
                    if (ex) {
                        callback(null, ex);
                        return;
        ...
        getLoadBalancer(callback: (name: string, error: e.Contracts.Error) => void): void {
            var params = {
                AutoScalingGroupName: this.autoScaleGroupName,
                MaxRecords: 2,
            };
        ...
        getGroupMinSize(callback: (minSize: number, error: e.Contracts.Error) => void): void {
            var params = {
        ...
        clearAllScheduledActions(callback: (err: e.Contracts.Error) => void): void {
            var errFactory = new e.Contracts.ErrorFactory();
            var me = this;
            this.getAllScheduledActions()
                ...
                .then(function (data) {
                    if (data.ScheduledUpdateGroupActions && data.ScheduledUpdateGroupActions.length > 0) {
                        var deletePromises = new Array<Object>();
        ...
        private deleteScheduledAction(actionName: string): any {
            var params = {
        ...
        private getAllScheduledActions(): any {
        ...
        setGroupInstanceSchedule(maxSpotPerHour: number, schedule: c.Contracts.Schedule, callback: (err: e.Contracts.Error) => void): void {
            var createPromises = new Array<Object>();
        ...
        private createSchedule(hour: number, instances: number, maxSpotPerHour: number): any {
        ...
        private getTimeAroundHourInFuture(hour: number): Date {
            var present = new Date();
    ...
    export class ScaleActivity {
        public isLaunchActivity: boolean = false;
        public isTerminationActivity: boolean = false;

        constructor(public startTime: Date, public endTime: Date, public statusCode: string,
            public progress: number, public description: string) {
